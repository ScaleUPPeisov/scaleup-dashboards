...
const shortsProduction=shorts.slice(0,shorts.indexOf('#[cfg(test)]'));
describe('VYRON 2.1.4 real-Mac follow-up contracts',()=>{
 it('Shorts selects an audible/default stream instead of hardcoding 0:a:0',()=>{expect(shorts).toContain('select_audible_audio_stream');expect(shorts).toContain('is_default');expect(shorts).toContain('volumedetect');expect(shortsProduction).not.toContain('"0:a:0"')});
