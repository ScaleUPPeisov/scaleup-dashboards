...
fn plan_build(req:&BuildRequest)->Result<BuildPlan,String>{
    if req.project_count==0||req.project_count>1000{return Err("Количество проектов должно быть 1–1000".into());}
...
pub fn validate_production_batch(manifest_path:String,endlume_path:String)->Result<Validation,String>{
    ...
    for p in &m.projects{
        ...
        let images=fs::read_dir(folder).ok().into_iter().flatten().filter_map(Result::ok).map(|e|e.path()).filter(|x|x.is_file()&&is_image(x)).collect::<Vec<_>>();
