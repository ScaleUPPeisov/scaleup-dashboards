...
async function ensureFolder(channel:Channel,job:VideoJob,projectRoot:string){
...
async function processChannel(channel:Channel,summary:AutopilotSummary,aiBudget:{left:number}){
  ...
  for(const job of jobs){
    ...
    try{const folder=await ensureFolder(channel,job,projectRoot);if(folder&&!job.folder)summary.prepared++}catch(e){useApp.getState().patchJob(job.id,{status:'ERROR',error:String(e)});logError(summary,`${channel.name} Video_${job.number}: папка`,e)}
  }
...
export async function runAutopilotCycle(manual=false):Promise<AutopilotSummary>{
  ...
  try{
    ...
    if(moved||summary.errors)useApp.getState().log(`Autopilot: папки ${summary.prepared}, треки ${summary.tracksMoved}, изображения ${summary.imagesMoved}, AI ${summary.metadataGenerated}, рендер ${summary.renderQueued}, ошибок ${summary.errors}`,summary.errors?'warn':'info');
