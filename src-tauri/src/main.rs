...
fn parse_srt(path: &Path) -> Result<Vec<Caption>, String> {
    ...
    for block in raw.split("\n\n") {
        ...
        let pair:Vec<&str>=ts.split(" --> ").collect();
...
async fn process_video(input: String, aspect: String, captions: bool) -> Result<String, String> {
    ...
    if captions {
        ...
        c.arg("extract-audio").arg(&input_path).arg(&audio);
        run(c,"Извлечение аудио")?;
        let mut a=Command::new("/usr/bin/afconvert");
        a.args(["-f","WAVE","-d","LEI16@16000","-c","1"]).arg(&audio).arg(&wav);
        run(a,"Подготовка аудио")?;
        let model=ensure_model()?;
        let whisper=bin_path("whisper-cli")?;
        let prefix=work.join("transcript");
        let mut w=Command::new(whisper);
        w.args(["-m"]).arg(model).arg("-f").arg(&wav).args(["-l","auto","-osrt","-of"]).arg(&prefix);
        run(w,"Распознавание речи")?;
        let srt=PathBuf::from(format!("{}.srt",prefix.display()));
        let parsed=parse_srt(&srt)?;
        fs::write(&captions_json,serde_json::to_vec(&parsed).map_err(|e|e.to_string())?).map_err(|e|e.to_string())?;
    } else {
        fs::write(&captions_json,"[]").map_err(|e|e.to_string())?;
    }
    let mut r=Command::new(&helper);
    r.arg("render").arg(&input_path).arg(&output).arg(&captions_json).arg(&aspect);
    run(r,"Экспорт видео")?;
