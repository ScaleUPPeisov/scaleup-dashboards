...
const AUDIO_EXT: &[&str] = &[
    "mp3", "wav", "m4a", "aac", "flac", "ogg", "opus", "aiff", "aif", "alac",
];
...
fn is_image(p: &Path) -> bool {
    IMAGE_EXT.contains(&ext(p).as_str())
}
fn is_audio(p: &Path) -> bool {
    AUDIO_EXT.contains(&ext(p).as_str())
}
fn nonempty_image(p: &Path) -> bool {
    p.is_file() && is_image(p) && fs::metadata(p).map(|m| m.len() > 0).unwrap_or(false)
}
fn project_has_renderable_image(folder: &Path, manifest_image: &Path) -> bool {
    if nonempty_image(manifest_image) {
        return true;
    }
    fs::read_dir(folder)
        .ok()
...
fn recursive_images(root: &Path) -> Result<Vec<PathBuf>, String> {
    ...
    while let Some(dir) = stack.pop() {
        let rd = fs::read_dir(&dir).map_err(|e| {
            if dir == root {
                format!("VYRON не может читать папку Downloads: {e}")
            } else {
                format!("Не удалось прочитать {}: {e}", dir.display())
            }
        })?;
...
fn image_snapshot(dir: &Path) -> HashSet<String> {
    recursive_images(dir)
        .unwrap_or_default()
        .into_iter()
...
fn recursive_audio(root: &Path) -> Vec<PathBuf> {
    ...
    while let Some(dir) = stack.pop() {
        if let Ok(rd) = fs::read_dir(dir) {
            for e in rd.flatten() {
                let p = e.path();
                if e.file_type().map(|x| x.is_symlink()).unwrap_or(false) {
                    continue;
                }
                if p.is_dir() {
                    stack.push(p)
...
pub struct ImportSession {
    ...
    pub collected: Vec<CollectedImage>,
}
#[derive(Clone, Debug, Serialize, Deserialize, Default)]
#[serde(rename_all = "camelCase")]
...
pub struct MusicHistory {
    pub schema_version: u32,
    pub tracks: HashMap<String, TrackUsage>,
    pub sequence_fingerprints: Vec<String>,
}
#[derive(Clone, Debug, Serialize, Deserialize, Default)]
#[serde(rename_all = "camelCase")]
...
pub struct ManifestProject {
    ...
    pub status: String,
    pub sequence_fingerprint: String,
}
#[derive(Clone, Debug, Serialize, Deserialize, Default)]
#[serde(rename_all = "camelCase")]
...
pub struct BatchProjectStatus {
    ...
    #[serde(default)]
    pub handoff_sent_at: Option<String>,
    #[serde(default)]
...
pub struct BatchStatus {
    ...
    pub status: String,
    pub updated_at: String,
    pub projects: Vec<BatchProjectStatus>,
}
#[derive(Clone, Debug, Serialize, Deserialize, Default)]
#[serde(rename_all = "camelCase")]
...
#[serde(rename_all = "camelCase")]
pub struct ArchiveRenderedResult {
...
pub struct HandoffReceipt {
    ...
    pub repeated_project_ids: Vec<String>,
}
#[derive(Clone, Debug, Serialize, Deserialize, Default)]
#[serde(rename_all = "camelCase")]
struct HandoffProjectLedger {
    sent_at: String,
    count: u32,
}
#[derive(Clone, Debug, Serialize, Deserialize, Default)]
...
struct HandoffLedger {
    schema_version: u32,
    projects: HashMap<String, HandoffProjectLedger>,
}
#[derive(Clone, Debug, Serialize, Deserialize, Default)]
#[serde(rename_all = "camelCase")]
...
#[derive(Clone, Debug, Serialize, Deserialize, Default)]
#[serde(rename_all = "camelCase")]
pub struct ProductionStorageStatus {
    pub path: String,
    pub exists: bool,
    pub writable: bool,
    pub external: bool,
...
fn storage_probe(path: &Path) -> ProductionStorageStatus {
    ...
    if !path.exists() {
        return ProductionStorageStatus {
            ...
            external,
            free_bytes: None,
            error: Some("Production-диск или папка недоступны".into()),
        };
    }
    if !path.is_dir() {
        return ProductionStorageStatus {
            ...
            external,
            free_bytes: None,
            error: Some("Production Workspace должен быть папкой".into()),
        };
    }
    ...
    let writable = std::fs::OpenOptions::new()
        ...
        .unwrap_or(false);
    ProductionStorageStatus {
        path: display,
        exists: true,
        writable,
        external,
        free_bytes: storage_free_bytes(path),
        error: if writable {
            None
        } else {
            Some("Нет доступа на запись в Production Workspace".into())
        },
    }
}
fn resolve_output_workspace(req: &BuildRequest) -> Result<String, String> {
    let chosen = req
        ...
        .map(str::trim)
        .filter(|x| !x.is_empty())
        .unwrap_or(req.workspace.trim());
    if chosen.is_empty() {
        return Err("Production Workspace не выбран".into());
    }
    if chosen != req.workspace.trim() {
        let st = storage_probe(Path::new(chosen));
        if !st.exists || !st.writable {
            return Err(st
                .error
                .unwrap_or_else(|| format!("Production Workspace недоступен: {chosen}")));
        }
    }
    Ok(chosen.to_string())
}
#[tauri::command]
pub fn production_storage_status(path: String) -> ProductionStorageStatus {
    let raw = PathBuf::from(path.trim());
    let exact = raw.canonicalize().unwrap_or(raw);
    storage_probe(&exact)
}
...
fn session_path(workspace: &str, channel_id: &str) -> Result<PathBuf, String> {
    Ok(channel_root(workspace, channel_id)?.join("import-session.json"))
}
fn settings_path(workspace: &str, channel_id: &str) -> Result<PathBuf, String> {
    Ok(channel_root(workspace, channel_id)?.join("settings.json"))
}
fn index_path(workspace: &str, channel_id: &str) -> Result<PathBuf, String> {
    Ok(channel_root(workspace, channel_id)?.join("music-index.json"))
}
fn history_path(workspace: &str, channel_id: &str) -> Result<PathBuf, String> {
    Ok(channel_root(workspace, channel_id)?.join("music-history.json"))
}
fn requests_path(workspace: &str, channel_id: &str) -> Result<PathBuf, String> {
    Ok(channel_root(workspace, channel_id)?.join("build-requests.json"))
}
...
fn normalize_import_runtime(
    ...
    if session.active && !import_runtime_active(channel_id) {
        ...
        if let Ok(path) = session_path(workspace, channel_id) {
            let _ = atomic_json(&path, &session);
        }
    }
    session
}
...
fn collector_seen_at_start(
    session: &ImportSession,
    _startup_snapshot: &HashSet<String>,
) -> HashSet<String> {
    ...
    session
        .collected
        .iter()
        .map(|x| x.source_path.clone())
        .collect()
}

fn spawn_import_watcher(
    app: AppHandle,
    workspace: String,
    channel_id: String,
    mut session: ImportSession,
    ...
    import_stops()
        .lock()
        .map_err(|_| "Import lock".to_string())?
        .insert(channel_id.clone(), stop.clone());
    ...
    eprintln!(
        "[image-collector] session started channel={} session={} downloads={} restored={}",
        channel_id,
        session.session_id,
        downloads.display(),
    ...
    thread::spawn(move || {
        ...
        while !stop.load(Ordering::SeqCst) {
            ...
            for src in files {
                ...
                if !import_candidate_ready(&src, &mut pending) {
                    continue;
                }
                ...
                let after = match fs::metadata(&src) {
                    Ok(x) => x,
                    Err(_) => {
                        let _ = fs::remove_file(&tmp);
                        continue;
                ...
                if after.len() != before_size
                    || modified_ms(&after) != before_modified
                    || copied != before_size
                {
                    let _ = fs::remove_file(&tmp);
                    pending.insert(
                ...
                eprintln!(
                    "[image-collector] imported channel={} session={} count={} source={}",
                    channel_id,
                    session.session_id,
                    session.collected.len(),
                    session
...
#[tauri::command]
pub fn start_production_import(
    app: AppHandle,
    workspace: String,
    channel_id: String,
    channel_name: String,
    ...
    let (session, baseline) =
        if old.active && !old.session_id.is_empty() && Path::new(&old.import_path).is_dir() {
            (old, image_snapshot(&downloads))
        } else {
            let sid = Uuid::new_v4().to_string();
            let import_dir = croot.join("Imports").join(&sid);
            ...
            (
                ImportSession {
                    ...
                    stopped_at: None,
                    downloads_path: downloads.to_string_lossy().into_owned(),
                    import_path: import_dir.to_string_lossy().into_owned(),
                    collected: Vec::new(),
                },
                image_snapshot(&downloads),
            )
        };
    let mut active = session.clone();
    active.active = true;
    active.stopped_at = None;
    atomic_json(&sp, &active)?;
    spawn_import_watcher(app, workspace, channel_id, active.clone(), baseline)?;
    Ok(active)
}

#[tauri::command]
pub fn stop_production_import(
    workspace: String,
    channel_id: String,
) -> Result<ImportSession, String> {
    if let Ok(map) = import_stops().lock() {
        if let Some(x) = map.get(&channel_id) {
...
pub fn production_import_status(
    ...
    let s: ImportSession = read_json(&session_path(&workspace, &channel_id)?);
    Ok(normalize_import_runtime(&workspace, &channel_id, s))
}

#[tauri::command]
pub fn set_production_music_library(
    workspace: String,
    channel_id: String,
    channel_name: String,
    path: String,
    ...
    let p = PathBuf::from(path)
        .canonicalize()
        .map_err(|_| "Папка музыкальной библиотеки не найдена".to_string())?;
    if !p.is_dir() {
        return Err("Музыкальная библиотека должна быть папкой".into());
    }
    let s = ChannelSettings {
        schema_version: SCHEMA_VERSION,
        channel_id: channel_id.clone(),
        channel_name,
        music_library: p.to_string_lossy().into_owned(),
        updated_at: Utc::now().to_rfc3339(),
    };
    atomic_json(&settings_path(&workspace, &channel_id)?, &s)?;
    Ok(s)
}
fn index_music_sync(workspace: &str, channel_id: &str) -> Result<MusicIndex, String> {
    ...
    if s.music_library.is_empty() {
        return Err("Сначала выбери папку музыки".into());
    }
    let lib = PathBuf::from(&s.music_library)
        .canonicalize()
        .map_err(|_| "Музыкальная библиотека недоступна".to_string())?;
    let old: MusicIndex = read_json(&index_path(workspace, channel_id)?);
    let old_map = old
        .tracks
        .into_iter()
    ...
    for p in recursive_audio(&lib) {
        ...
        let mm = modified_ms(&m);
        if let Some(prev) = old_map
            .get(&ps)
            .filter(|x| x.size == m.len() && x.modified_ms == mm)
        {
            tracks.push(prev.clone());
            continue;
        }
        tracks.push(TrackIndex {
            track_id: hash_file(&p)?,
            path: ps,
            size: m.len(),
            modified_ms: mm,
    ...
    tracks.sort_by(|a, b| {
        let an = Path::new(&a.path)
            .file_name()
            .and_then(|x| x.to_str())
            .unwrap_or("")
            .to_lowercase();
        let bn = Path::new(&b.path)
            .file_name()
            .and_then(|x| x.to_str())
            .unwrap_or("")
            .to_lowercase();
    ...
    atomic_json(&index_path(workspace, channel_id)?, &idx)?;
    Ok(idx)
}
#[tauri::command]
pub async fn index_production_music_library(
    workspace: String,
    channel_id: String,
) -> Result<MusicSummary, String> {
    let idx =
        tauri::async_runtime::spawn_blocking(move || index_music_sync(&workspace, &channel_id))
            .await
            .map_err(|e| e.to_string())??;
    Ok(MusicSummary {
        library_path: idx.library_path,
...
fn next_batch_id(parent: &Path, channel_name: &str) -> String {
    ...
    let prefix = format!("{}_BATCH_{}_", safe_component(channel_name), date);
    let mut max = 0u32;
    if let Ok(rd) = fs::read_dir(parent) {
        for e in rd.flatten() {
            let n = e.file_name().to_string_lossy().to_string();
            if let Some(x) = n.strip_prefix(&prefix).and_then(|x| x.parse::<u32>().ok()) {
                max = max.max(x);
            }
        }
    }
...
fn choose_sequence(
    ...
    planned: &HashMap<String, u64>,
    global_pool: &mut Vec<usize>,
    cursor: &mut usize,
    count: usize,
    batch_id: &str,
    project_no: usize,
    ...
    if tracks.len() < count {
        return Err(format!(
            "В библиотеке {} уникальных треков, на проект нужно {}",
            tracks.len(),
            count
        ));
    }
    for attempt in 0..100 {
        let salt = format!("{batch_id}:{project_no}:{attempt}:{mode}");
        let mut picked = Vec::new();
        match mode {
            "even" => {
                let mut s = (0..tracks.len())
                    .map(|i| {
                        let id = &tracks[i].track_id;
                        let used = history.tracks.get(id).map(|x| x.times_used).unwrap_or(0)
                            + planned.get(id).copied().unwrap_or(0);
                        (used, seed64(&format!("{salt}:{id}")), i)
                    })
                    .collect::<Vec<_>>();
                s.sort_by_key(|x| (x.0, x.1));
                picked = s.into_iter().take(count).map(|x| x.2).collect();
            }
            "alphabetical" => {
            ...
            "no-repeat" => {
                let mut local = HashSet::new();
                while picked.len() < count {
                    if *cursor >= global_pool.len() {
                        *global_pool = (0..tracks.len()).collect();
                        shuffle(global_pool, seed64(&format!("{salt}:cycle")));
                        *cursor = 0;
                    }
                    let i = global_pool[*cursor];
                    *cursor += 1;
                    if local.insert(i) {
                        picked.push(i);
                    }
            ...
            _ => {
                let mut p = (0..tracks.len()).collect::<Vec<_>>();
                shuffle(&mut p, seed64(&salt));
                picked = p.into_iter().take(count).collect();
            }
        }
        if mode != "alphabetical" {
            shuffle(&mut picked, seed64(&format!("{salt}:order")));
        }
        let mut ids = picked
            .iter()
            .map(|i| tracks[*i].track_id.clone())
            .collect::<Vec<_>>();
        if mode == "alphabetical" && seen.contains(&seq_hash(&ids)) && picked.len() > 1 {
            let n = picked.len();
            picked.rotate_left((project_no + attempt + 1) % n);
            ids = picked.iter().map(|i| tracks[*i].track_id.clone()).collect();
        }
        if !seen.contains(&seq_hash(&ids)) {
            return Ok(picked);
        }
    }
...
fn plan_build(req: &BuildRequest) -> Result<BuildPlan, String> {
    ...
    if !matches!(
        req.mode.as_str(),
        "even" | "random" | "alphabetical" | "no-repeat"
    ) {
    ...
    if idx.tracks.len() < req.tracks_per_project {
        return Err(format!(
            "В библиотеке недостаточно уникальных треков: {} < {}",
            idx.tracks.len(),
    ...
    let mut seen = history
        ...
        .cloned()
        .collect::<HashSet<_>>();
    let mut planned = HashMap::<String, u64>::new();
    let mut pool = (0..idx.tracks.len()).collect::<Vec<_>>();
    shuffle(&mut pool, seed64(&bid));
    ...
    for i in 0..req.project_count {
        ...
        let ids = picks
            .iter()
            .map(|x| idx.tracks[*x].track_id.clone())
            .collect::<Vec<_>>();
        let fp = seq_hash(&ids);
        ...
        for (pos, ti) in picks.iter().enumerate() {
            let t = &idx.tracks[*ti];
            *planned.entry(t.track_id.clone()).or_insert(0) += 1;
            let src = Path::new(&t.path);
            let stem = src.file_stem().and_then(|x| x.to_str()).unwrap_or("track");
            let ex = ext(src);
            pt.push(PlanTrack {
                track_id: t.track_id.clone(),
                source: t.path.clone(),
                duration_sec: t.duration_sec,
                dest_name: format!("{:03}_{}.{}", pos + 1, safe_component(stem), ex),
            });
        }
    ...
    atomic_json(
        ...
        &Checkpoint {
            ...
            total_projects: req.project_count,
...
fn project_ready(plan: &PlanProject, folder: &Path) -> bool {
    ...
    let image = folder.join(&plan.image_name);
    if !image.is_file() || fs::metadata(&image).map(|m| m.len() == 0).unwrap_or(true) {
        return false;
    }
    plan.tracks.iter().all(|t| {
        let p = folder.join(&t.dest_name);
...
fn execute_plan(app: Option<&AppHandle>, plan: &BuildPlan) -> Result<BatchSummary, String> {
    ...
    let mut manifest = BatchManifest {
        schema_version: SCHEMA_VERSION,
        source: "VYRON Production Manager".into(),
        batch_id: plan.batch_id.clone(),
        channel_id: plan.request.channel_id.clone(),
        channel_name: plan.request.channel_name.clone(),
        created_at: plan.created_at.clone(),
        project_count: plan.projects.len(),
        tracks_per_project: plan.request.tracks_per_project,
        mode: plan.request.mode.clone(),
        root_path: plan.batch_root.clone(),
        output_dir: broot.join("Rendered").to_string_lossy().into_owned(),
        status_path: broot.join("status.json").to_string_lossy().into_owned(),
        status: "Готово".into(),
        projects: Vec::new(),
    };
    fs::create_dir_all(&manifest.output_dir).map_err(|e| e.to_string())?;
    for p in &plan.projects {
        let folder = broot.join(&p.project_id);
        let tracks = p
            .tracks
            .iter()
            .map(|t| ManifestTrack {
                track_id: t.track_id.clone(),
                path: folder.join(&t.dest_name).to_string_lossy().into_owned(),
        ...
        manifest.projects.push(ManifestProject {
            ...
            tracks,
            status: "Waiting".into(),
            sequence_fingerprint: p.sequence_fingerprint.clone(),
        });
    }
    ...
    if !cp.history_applied {
        ...
        if h.sequence_fingerprints.len() > 10000 {
            let n = h.sequence_fingerprints.len() - 10000;
            h.sequence_fingerprints.drain(0..n);
        }
        atomic_json(&hp, &h)?;
    ...
    let status = BatchStatus {
        ...
        projects: manifest
            .projects
            .iter()
            .map(|p| BatchProjectStatus {
                project_id: p.project_id.clone(),
                job_id: p.job_id.clone(),
                video_number: p.video_number,
                render_status: "Waiting".into(),
                output_file: None,
                duration: None,
                file_size: None,
                error: None,
...
fn recovery_in_workspace(workspace: &str) -> Vec<RecoveryState> {
    ...
    for channel in channels.flatten() {
        ...
        let batches = match fs::read_dir(channel.path()) {
            Ok(x) => x,
            Err(_) => continue,
        };
        for batch in batches.flatten() {
            let broot = batch.path();
            if !broot.is_dir() {
                continue;
            }
            let plan: BuildPlan = read_json(&broot.join("plan.json"));
            if plan.batch_id.is_empty() {
                continue;
            }
            let mut cp: Checkpoint = read_json(&broot.join("checkpoint.json"));
            if cp.total_projects == 0 {
                cp.total_projects = plan.projects.len();
            }
            if matches!(
                cp.status.as_str(),
                "Готово" | "Заменено новой сборкой" | "Отменено"
            ) {
                continue;
            }
            let actual = plan
                .projects
                .iter()
                .filter(|p| project_ready(p, &broot.join(&p.project_id)))
                .count();
            if actual != cp.completed_projects {
                cp.completed_projects = actual;
                cp.updated_at = Utc::now().to_rfc3339();
                let _ = atomic_json(&broot.join("checkpoint.json"), &cp);
            }
            let current = if actual < plan.projects.len() {
                format!("{:03}", actual + 1)
            } else {
                "Финализация".into()
            };
            out.push(RecoveryState {
                batch_id: plan.batch_id.clone(),
                channel_id: plan.request.channel_id.clone(),
                channel_name: plan.request.channel_name.clone(),
                root_path: broot.to_string_lossy().into_owned(),
...
pub fn find_production_recovery(workspaces: Vec<String>) -> Result<Vec<RecoveryState>, String> {
    ...
    for raw in workspaces {
        ...
        out.extend(recovery_in_workspace(w));
    }
...
#[tauri::command]
pub async fn restart_production_batch(
    app: AppHandle,
    manifest_or_root: String,
) -> Result<BatchSummary, String> {
    tauri::async_runtime::spawn_blocking(move || {
        let _g = build_lock().lock().map_err(|_| "Build lock".to_string())?;
        let old_root = if Path::new(&manifest_or_root).is_dir() {
            PathBuf::from(&manifest_or_root)
        } else {
            Path::new(&manifest_or_root)
                .parent()
                .unwrap_or(Path::new("."))
                .to_path_buf()
        };
        let old_plan: BuildPlan = read_json(&old_root.join("plan.json"));
        if old_plan.batch_id.is_empty() {
            return Err("Не найден план прерванной сборки".into());
        }
        let mut request = old_plan.request.clone();
        request.request_id = Uuid::new_v4().to_string();
        let new_plan = plan_build(&request)?;
        let old_cp_path = old_root.join("checkpoint.json");
        let mut old_cp: Checkpoint = read_json(&old_cp_path);
        old_cp.status = "Заменено новой сборкой".into();
        old_cp.updated_at = Utc::now().to_rfc3339();
        atomic_json(&old_cp_path, &old_cp)?;
...
fn summary_from(m: &BatchManifest, s: &BatchStatus) -> BatchSummary {
    BatchSummary {
        ...
        tracks_assigned: m.projects.iter().map(|p| p.tracks.len()).sum(),
        status: s.status.clone(),
        manifest_path: Path::new(&m.root_path)
            .join("batch.json")
            .to_string_lossy()
            .into_owned(),
        ...
        completed_projects: s
            .projects
            .iter()
            .filter(|p| p.render_status == "Completed")
            .count(),
        error_projects: s
            .projects
            .iter()
            .filter(|p| p.render_status == "Error")
            .count(),
    }
}
fn request_map(workspace: &str, channel_id: &str) -> Result<HashMap<String, String>, String> {
    Ok(read_json(&requests_path(workspace, channel_id)?))
}
fn save_request_map(
    workspace: &str,
    channel_id: &str,
    ...
    atomic_json(&requests_path(workspace, channel_id)?, m)
}
fn load_manifest(path_or_root: &str) -> Result<(BatchManifest, PathBuf), String> {
    let p = PathBuf::from(path_or_root);
    let mp = if p.is_dir() { p.join("batch.json") } else { p };
    let m: BatchManifest =
        serde_json::from_slice(&fs::read(&mp).map_err(|_| "batch.json не найден".to_string())?)
...
pub async fn build_production_batch(
    ...
    if request.project_count > available && !request.allow_image_reuse {
        return Ok(BuildResult {
            ...
            requested_projects: request.project_count,
            batch: None,
            message: Some(format!(
                "Доступно {available}, запрошено {}",
...
pub async fn resume_production_batch(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let plan: BuildPlan = read_json(&root.join("plan.json"));
        if plan.batch_id.is_empty() {
            let (m, _) = load_manifest(&manifest_or_root)?;
            let s: BatchStatus = read_json(Path::new(&m.status_path));
            return Ok(summary_from(&m, &s));
        }
        execute_plan(Some(&app), &plan)
    })
    .await
    .map_err(|e| e.to_string())?
}
fn handoff_ledger_path(m: &BatchManifest) -> PathBuf {
    PathBuf::from(&m.root_path).join("vyron-handoff.json")
}
...
fn enrich_handoff(mut status: BatchStatus, m: &BatchManifest) -> BatchStatus {
    ...
    for row in &mut status.projects {
        if let Some(x) = ledger.projects.get(&row.project_id) {
            row.handoff_status = Some("SENT".into());
            row.handoff_sent_at = Some(x.sent_at.clone());
            row.handoff_count = Some(x.count);
        }
    }
    status
}
...
pub fn list_production_batches(
    ...
    let parent = batch_root_parent(&workspace, &channel_id)?;
    let mut out = Vec::new();
    if let Ok(rd) = fs::read_dir(parent) {
        for e in rd.flatten() {
            let p = e.path();
            ...
            if let Ok((m, _)) = load_manifest(&p.to_string_lossy()) {
                let s: BatchStatus = read_json(Path::new(&m.status_path));
                out.push(summary_from(&m, &s));
            }
        }
    }
...
#[tauri::command]
pub fn production_channel_state(
    workspace: String,
    channel_id: String,
) -> Result<ChannelState, String> {
    let settings: ChannelSettings = read_json(&settings_path(&workspace, &channel_id)?);
    let import_session: ImportSession = normalize_import_runtime(
        &workspace,
        &channel_id,
        read_json(&session_path(&workspace, &channel_id)?),
    );
    let idx: MusicIndex = read_json(&index_path(&workspace, &channel_id)?);
    let music = if idx.library_path.is_empty() {
        None
    } else {
        Some(MusicSummary {
            library_path: idx.library_path,
...
fn validate_manifest_projects(
    ...
    for p in &m.projects {
        if selected
            .map(|ids| !ids.contains(&p.project_id))
            .unwrap_or(false)
        {
            continue;
        }
        let mut errs = Vec::new();
        let folder = Path::new(&p.folder_path);
        if !folder.is_dir() {
            errs.push("папка проекта не найдена".to_string());
        }
        ...
        if p.tracks.len() != m.tracks_per_project {
            errs.push(format!(
                "треков: {}, должно быть {}",
                p.tracks.len(),
                m.tracks_per_project
            ));
        }
        for t in &p.tracks {
            let q = Path::new(&t.path);
            if !q.is_file() {
                errs.push(format!(
                    "нет {}",
                    q.file_name().and_then(|x| x.to_str()).unwrap_or("track")
                ));
            } else if fs::metadata(q).map(|x| x.len() == 0).unwrap_or(true) {
                errs.push("найден 0-byte аудиофайл".into());
            }
            if !is_audio(q) {
                errs.push("неподдерживаемый аудиоформат".into());
            }
        }
        if !fps.insert(p.sequence_fingerprint.clone()) {
            errs.push("повтор музыкальной последовательности".into());
        }
    ...
    let ready = items.iter().filter(|x| x.ok).count();
    let errors = items.len() - ready;
    Validation {
        batch_id: m.batch_id.clone(),
        ready,
        errors,
        endlume_exists: Path::new(endlume_path).exists(),
        items,
    }
}
#[tauri::command]
pub fn validate_production_batch(
    manifest_path: String,
    endlume_path: String,
) -> Result<Validation, String> {
    let (m, _) = load_manifest(&manifest_path)?;
    Ok(validate_manifest_projects(&m, &endlume_path, None))
}
#[tauri::command]
pub fn validate_production_projects(
    manifest_path: String,
    endlume_path: String,
    project_ids: Vec<String>,
) -> Result<Validation, String> {
    let (m, _) = load_manifest(&manifest_path)?;
    let ids = project_ids.into_iter().collect::<HashSet<_>>();
    if ids.is_empty() {
        return Err("Не выбраны проекты для ENDLUME".into());
    }
    if ids
        .iter()
        .any(|id| !m.projects.iter().any(|p| &p.project_id == id))
    {
        return Err("Выбранный проект отсутствует в batch".into());
    }
...
fn safe_cleanup_root(root: &Path) -> Result<PathBuf, String> {
    let c = root
        .canonicalize()
        .map_err(|_| "Workspace недоступен".to_string())?;
    if c == PathBuf::from("/") {
...
fn canonical_under(root: &Path, path: &Path) -> Result<PathBuf, String> {
    let r = safe_cleanup_root(root)?;
    let p = path
        .canonicalize()
        .map_err(|_| format!("Путь не найден: {}", path.display()))?;
    if !p.starts_with(&r) || p == r {
        return Err("Отказано: путь проекта находится вне безопасного VYRON root".into());
    }
    Ok(p)
}
#[tauri::command]
...
fn folder_stats(path: &Path) -> (usize, u64) {
        ...
        if let Ok(rd) = fs::read_dir(dir) {
            for e in rd.flatten() {
                ...
                if ft.is_dir() {
                    ...
                    files += 1;
                    bytes = bytes.saturating_add(e.metadata().map(|m| m.len()).unwrap_or(0));
                }
            }
        }
    }
...
#[derive(Clone, Debug, Serialize, Deserialize, Default)]
#[serde(rename_all = "camelCase")]
pub struct GlobalProjectCleanupPreview {
...
#[derive(Clone, Debug, Serialize, Deserialize, Default)]
#[serde(rename_all = "camelCase")]
pub struct GlobalProjectCleanupResult {
    pub scanned_channels: usize,
    pub found_projects: usize,
...
fn global_manifest_paths(workspaces: &[String]) -> Vec<PathBuf> {
    let mut out = Vec::new();
...
pub fn archive_production_rendered_videos(
    manifest_path: String,
    archive_root: String,
) -> Result<ArchiveRenderedResult, String> {
    if archive_root.trim().is_empty() {
        return Err("Папка архива не выбрана".into());
    }
    let (m, _) = load_manifest(&manifest_path)?;
    let st: BatchStatus = read_json(Path::new(&m.status_path));
    let batch_root = PathBuf::from(&m.root_path)
        .canonicalize()
        .map_err(|e| format!("Batch недоступен: {e}"))?;
    let target = PathBuf::from(archive_root.trim())
        .join(safe_component(&m.channel_name))
        .join(safe_component(&m.batch_id));
    fs::create_dir_all(&target).map_err(|e| format!("Не удалось создать архив: {e}"))?;
    let mut out = ArchiveRenderedResult {
        archive_path: target.to_string_lossy().into_owned(),
        ..Default::default()
    };
    for row in &st.projects {
        ...
        let Some(src_raw) = row.output_file.as_ref() else {
            out.skipped_files += 1;
            continue;
        };
        ...
        let canon = src
            .canonicalize()
            .map_err(|e| format!("MP4 недоступен: {e}"))?;
        if !canon.starts_with(&batch_root) {
            return Err(format!("BLOCK: MP4 вне batch: {}", canon.display()));
        }
        let name = canon
            .file_name()
            .ok_or_else(|| "Некорректное имя MP4".to_string())?;
        let dst = target.join(name);
        let bytes = fs::metadata(&canon).map_err(|e| e.to_string())?.len();
        if dst.is_file() && fs::metadata(&dst).map(|x| x.len()).unwrap_or(0) == bytes {
            out.skipped_files += 1;
            continue;
        }
        let tmp = target.join(format!(".{}.part", name.to_string_lossy()));
        if tmp.exists() {
            let _ = fs::remove_file(&tmp);
        }
        fs::copy(&canon, &tmp)
...
fn delete_production_batch_projects_inner(
    ...
    if wanted.is_empty() {
        return Ok(DeleteResult {
            ...
            deleted_job_ids: Vec::new(),
            batch: Some(summary_from(&m, &read_json(Path::new(&m.status_path)))),
        });
    }
    let selected = m
        .projects
        .iter()
        .filter(|p| wanted.contains(&p.project_id))
        .cloned()
        .collect::<Vec<_>>();
    ...
    st.projects.retain(|p| !wanted.contains(&p.project_id));
    st.status = if st.projects.is_empty() {
        "Удалён".into()
    } else {
        "Готово".into()
    };
    st.updated_at = Utc::now().to_rfc3339();
    let broot = PathBuf::from(&m.root_path);
    let plan_path = broot.join("plan.json");
    if plan_path.exists() {
        let mut plan: BuildPlan = read_json(&plan_path);
        plan.projects.retain(|p| !wanted.contains(&p.project_id));
        plan.request.project_count = plan.projects.len();
        atomic_json(&plan_path, &plan)?;
        let cp_path = broot.join("checkpoint.json");
        let mut cp: Checkpoint = read_json(&cp_path);
        cp.total_projects = plan.projects.len();
        cp.completed_projects = cp.completed_projects.min(cp.total_projects);
        cp.updated_at = Utc::now().to_rfc3339();
        atomic_json(&cp_path, &cp)?;
    }
...
pub fn open_production_batch_in_endlume(
    ...
    force_resend: Option<bool>,
) -> Result<HandoffReceipt, String> {
    let (m, _) = load_manifest(&manifest_path)?;
    let app = PathBuf::from(&endlume_path);
    if !app.exists() {
    ...
    let idset = ids.iter().cloned().collect::<HashSet<_>>();
    let selected = m
        .projects
        .iter()
        .filter(|p| idset.contains(&p.project_id))
        .cloned()
        .collect::<Vec<_>>();
    ...
    let mut subset = m.clone();
    subset.projects = selected;
    subset.project_count = subset.projects.len();
    subset.status = "Передано ENDLUME".into();
    let subset_path = root.join(format!(
        ".vyron-handoff-{}.json",
...
pub fn production_endlume_handoff_consumed(request_path: String) -> Result<bool, String> {
    ...
    Ok(!Path::new(&request_path).exists())
}
...
mod v1015_image_validation_tests {
    ...
    fn selected_validation_does_not_fail_for_unselected_bad_project() {
        let root = temp_project("selected-validation");
        let good = root.join("001");
        let bad = root.join("002");
        fs::create_dir_all(&good).unwrap();
        fs::create_dir_all(&bad).unwrap();
        let img = good.join("image.png");
        fs::write(&img, b"png").unwrap();
        let track = good.join("a.mp3");
        fs::write(&track, b"mp3").unwrap();
        let manifest = BatchManifest {
            batch_id: "B".into(),
            root_path: root.to_string_lossy().into_owned(),
            tracks_per_project: 1,
            projects: vec![
                ManifestProject {
                    project_id: "001".into(),
                    folder_path: good.to_string_lossy().into_owned(),
                    image_path: img.to_string_lossy().into_owned(),
                    tracks: vec![ManifestTrack {
                        path: track.to_string_lossy().into_owned(),
                        ..Default::default()
                    }],
                    sequence_fingerprint: "one".into(),
                    ..Default::default()
                },
                ManifestProject {
                    project_id: "002".into(),
                    folder_path: bad.to_string_lossy().into_owned(),
                    image_path: bad.join("missing.png").to_string_lossy().into_owned(),
                    tracks: vec![],
                    sequence_fingerprint: "two".into(),
                    ..Default::default()
                },
            ],
            ..Default::default()
        };
        let ids = ["001".to_string()].into_iter().collect::<HashSet<_>>();
        let v =
            validate_manifest_projects(&manifest, "/Applications/ENDLUME Studio.app", Some(&ids));
        assert_eq!(v.items.len(), 1);
        assert_eq!(v.items[0].project_id, "001");
        assert!(v.items[0].ok);
    ...
    fn one_manifest_image_is_valid() {
        let d = temp_project("one");
        let img = d.join("image.png");
        fs::write(&img, b"png").unwrap();
        assert!(project_has_renderable_image(&d, &img));
        let _ = fs::remove_dir_all(d);
    }

    #[test]
    fn multiple_images_are_valid_for_endlume() {
        let d = temp_project("many");
        let img = d.join("image.png");
        fs::write(&img, b"png").unwrap();
        fs::write(d.join("transition.jpg"), b"jpg").unwrap();
        assert!(project_has_renderable_image(&d, &img));
        let _ = fs::remove_dir_all(d);
    }
    ...
    fn fallback_image_is_valid_if_manifest_path_was_changed() {
        let d = temp_project("fallback");
        fs::write(d.join("other.webp"), b"webp").unwrap();
