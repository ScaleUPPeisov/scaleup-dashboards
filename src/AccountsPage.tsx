    ...
    :<div className="accountList">{profiles.map(p=>{
      ...
      return <article id={`oauth-profile-${p.id}`} className="accountRow accountRowStats" key={p.id}>
       ...
       <div className="accountActions">
        ...
        <button className="mini" disabled={busy} title={credentialState==='KEYCHAIN_BLOCKED'?'Войти заново через браузер':'Переподключить через браузер'} aria-label={credentialState==='KEYCHAIN_BLOCKED'?'Войти заново через браузер':'Переподключить через браузер'} onClick={()=>void askBrowser(p.id)}>{credentialState==='KEYCHAIN_BLOCKED'?'Войти заново':'Переподключить'}</button>
