...
const badge=(s:FinalAuthStatus)=>s==='CONNECTED'?'✓ ПОДКЛЮЧЕНО':s==='CANONICAL_PRESENT_UNVERIFIED'?'○ CREDENTIAL ГОТОВ К ПРОВЕРКЕ':s==='RECONNECT REQUIRED'?'↻ ТРЕБУЕТ ПЕРЕПОДКЛЮЧЕНИЯ':s==='MISSING'?'↻ CREDENTIAL ОТСУТСТВУЕТ':s==='CONNECTING'?'… CONNECTING':s==='VALIDATING'?'… VALIDATING':s==='WRONG CHANNEL'?'⚠ WRONG CHANNEL':'✕ FAILED';
...
export function AuthRecoveryCenter(){
 ...
 const connected=rows.filter(x=>x.status==='CONNECTED').length,ready=rows.filter(x=>x.status==='CANONICAL_PRESENT_UNVERIFIED').length,errors=rows.filter(x=>x.status==='FAILED'||x.status==='WRONG CHANNEL').length+unmapped.length,target=(focus&&queue.find(x=>x.profileId===focus))||queue[0];
 ...
 return <section className="settingsCard authRecoveryCenter"><div className="panelHead"><div><small>EXISTING PROFILE RE-AUTH • SAFE RECOVERY</small><h3>Восстановление каналов</h3><p>Каналы, расписания, SEO и Production data не пересоздаются. Меняются только OAuth credentials существующего Profile UUID.</p></div><button className="settingsAction" onClick={()=>load()} disabled={!!busy}>Обновить статусы</button></div>
  <div className="settingsInfoGrid"><span><small>Всего каналов</small><b>{channels.length}</b></span><span><small>OAuth profiles</small><b>{profiles.length}</b></span><span><small>Подключены</small><b>{connected}</b></span><span><small>Готовы к проверке</small><b>{ready}</b></span><span><small>Требуют переподключения</small><b>{queue.length}</b></span><span><small>Ошибки mapping/auth</small><b>{errors}</b></span></div>
  {target&&<div className="settingsCard" style={{marginTop:12}}><small>ОЧЕРЕДЬ ПЕРЕПОДКЛЮЧЕНИЯ</small><h3>Профиль {currentNo} из {Math.max(total,1)}</h3><p><b>{target.channels.map(c=>c.name).join(' • ')}</b></p><p>Profile UUID: <code>{target.profileId}</code></p><p>Expected channel_id: <code>{target.expectedChannelId||'—'}</code></p><button className="primary" disabled={!!busy} onClick={()=>chooseBrowser(target.profileId)}>{busy===target.profileId?'Подключение…':'Переподключить следующий'}</button><p className="note">После validated success VYRON автоматически выбирает следующий missing profile. Google consent/login остаётся вашим действием.</p></div>}
  {!queue.length&&loaded&&errors===0&&ready===0&&<div className="successBox"><b>Восстановление завершено</b><br/>Channels: {channels.length} • OAuth profiles: {profiles.length} • Connected: {connected} • Reconnect required: 0</div>}
  <div className="recoveryProfileRows">{rows.map(r=><article key={r.profileId} className={r.status==='CONNECTED'?'good':(r.status==='RECONNECT REQUIRED'||r.status==='MISSING'||r.status==='CANONICAL_PRESENT_UNVERIFIED')?'warn':'bad'}><div><small>{r.channels.length?`${r.channels.length} channel${r.channels.length>1?'s':''}`:'STALE PROFILE'}</small><b>{r.channels.length?r.channels.map(c=>c.name).join(' • '):(r.profile?.channelTitle||'Без привязанного канала')}</b><p>Profile UUID: <code>{r.profileId}</code></p><p>Channel ID: <code>{r.expectedChannelId||'—'}</code></p><p>{r.detail}</p></div><div><strong>{badge(r.status)}</strong>{(r.status==='RECONNECT REQUIRED'||r.status==='MISSING')&&<button disabled={!!busy} onClick={()=>chooseBrowser(r.profileId)}>Переподключить</button>}{(r.status==='WRONG CHANNEL'||r.status==='FAILED')&&!r.stale&&!r.duplicate&&!r.conflict&&r.expectedChannelId&&<button disabled={!!busy} onClick={()=>{setTransient(x=>{const n={...x};delete n[r.profileId];return n});void chooseBrowser(r.profileId)}}>Повторить OAuth</button>}</div></article>)}</div>
