...
export function AuthRecoveryCenter(){
 ...
 async function importProfileCredentials(profileId:string){if(busy)return;setBusy(`credentials:${profileId}`);try{const result=await api.youtubeImportProfileCredentials(profileId);if(!result)return;const fresh=await api.youtubeOauthCredentialStates();setCredentialStates(fresh);setTransient(x=>{const n={...x};delete n[profileId];return n});notifySuccess('OAuth Client настроен',`credentials.json проверен по exact client_id и client_secret сохранён защищённо для Profile UUID ${profileId}.`)}catch(e){const h=String(e);notifyError('Не удалось импортировать OAuth Client',h,{persistError:false})}finally{setBusy('')}}
 ...
 return <section className="settingsCard authRecoveryCenter"><div className="panelHead"><div><small>EXISTING PROFILE RE-AUTH • SAFE RECOVERY</small><h3>Восстановление каналов</h3><p>Каналы, расписания, SEO и Production data не пересоздаются. Меняются только OAuth credentials существующего Profile UUID.</p></div><button className="settingsAction" onClick={()=>load()} disabled={!!busy}>Обновить статусы</button></div>
  ...
  <div className="recoveryProfileRows">{rows.map(r=><article key={r.profileId} className={r.status==='CONNECTED'?'good':(r.status==='RECONNECT REQUIRED'||r.status==='MISSING'||r.status==='CANONICAL_PRESENT_UNVERIFIED'||r.status==='CLIENT SECRET REQUIRED')?'warn':'bad'}><div><small>{r.channels.length?`${r.channels.length} channel${r.channels.length>1?'s':''}`:'STALE PROFILE'}</small><b>{r.channels.length?r.channels.map(c=>c.name).join(' • '):(r.profile?.channelTitle||'Без привязанного канала')}</b><p>Profile UUID: <code>{r.profileId}</code></p><p>Channel ID: <code>{r.expectedChannelId||'—'}</code></p><p>{r.detail}</p></div><div><strong>{badge(r.status)}</strong>{r.status==='CLIENT SECRET REQUIRED'&&<button disabled={!!busy} onClick={()=>void importProfileCredentials(r.profileId)}>{busy===`credentials:${r.profileId}`?'Импорт…':'Импортировать credentials.json'}</button>}{(r.status==='RECONNECT REQUIRED'||r.status==='MISSING')&&<button disabled={!!busy} onClick={()=>chooseBrowser(r.profileId)}>Переподключить</button>}{(r.status==='WRONG CHANNEL'||r.status==='FAILED')&&!r.stale&&!r.duplicate&&!r.conflict&&r.expectedChannelId&&<button disabled={!!busy} onClick={()=>{setTransient(x=>{const n={...x};delete n[r.profileId];return n});void chooseBrowser(r.profileId)}}>Повторить OAuth</button>}</div></article>)}</div>
