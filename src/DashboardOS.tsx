...
export function DashboardOS(){
 const channels=useApp(s=>s.channels),jobs=useApp(s=>s.jobs),uploadHistory=useApp(s=>s.uploadHistory),projectLifecycle=useApp(s=>s.projectLifecycle),fingerprintCache=useApp(s=>s.fingerprintCache),setPage=useApp(s=>s.setPage);
 ...
 const attention=useMemo(()=>{
  ...
  for(const c of enabled){const inv=inventory.byChannel[c.id];if(inv&&inv.stockDays===0)rows.push({key:`stock:${c.id}`,label:c.name,text:'0 дней запаса готовых видео',page:'production'});if(!isFutureChannel(c)&&!c.youtubeProfileId)rows.push({key:`oauth:${c.id}`,label:c.name,text:'требуется подключение YouTube OAuth',page:'youtube'})}
 ...
 },[enabled,inventory,jobErrors,history]);
