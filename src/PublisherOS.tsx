...
export function PublisherOS(){
 ...
 const stateCounts=uploadStateCounters(allChannelJobs,uploadHistory),newCount=selectableJobs.length,uploadedCount=stateCounts.ON_YOUTUBE,processingCount=stateCounts.PROCESSING,verifyCount=stateCounts.VERIFY_REQUIRED,errorCount=stateCounts.ERRORS,uploadedIds=allChannelJobs.filter(j=>['READY','UPLOAD_ACCEPTED','YOUTUBE_PROCESSING'].includes(stateOf(j))).map(j=>j.id);
 ...
 async function reconcileExistingUploads(targetJobIds?:string[]){
  ...
  try{
   ...
   let remoteExists=0,ready=0,processing=0,missing=0,unknown=0,repairs=0,nextHistory=[...useApp.getState().uploadHistory];
   for(const j of targets){
    ...
    if(row.remoteExists===true&&row.identityVerified){
     ...
     else if(j.uploadedAt&&j.finalPath){try{const fp=await fingerprintForJob(j);nextHistory=recordVerifiedUpload(nextHistory,{jobId:j.id,channelId,profileId,youtubeChannelId:expectedChannelId,youtubeVideoId:videoId,localFilePath:j.finalPath,originalFilename:baseName(j.finalPath),titleAtUpload:j.title,uploadedAt:j.uploadedAt,fileSize:fp.size,sha256:fp.fingerprint,publishAt:row.publishAt||j.publishAt,sourceLifecycle:'PRESENT',remoteExists:true,remoteCheckedAt:row.processingCheckedAt,processingState:row.processingState,processingCheckedAt:row.processingCheckedAt,processingStatus:row.processingStatus,processingError,readyAt:row.processingState==='READY'?row.processingCheckedAt:undefined,identityVerifiedAt:row.processingCheckedAt});repairs++}catch{}}
 ...
 async function chooseChannelRenderFolder(){
  ...
  setRenderScan(null);
  notifySuccess('Папка рендера канала сохранена',`${channel.name} → ${selected}. YouTube API: 0.`);
 }
 async function chooseChannelProjectsFolder(){
  ...
  notifySuccess('Папка проектов канала сохранена',`${channel.name} → ${selected}. YouTube API: 0.`);
 }
 ...
 async function bulkReconcileLegacyRenderRows(){
  if(!renderScan||!channel)return;
  const initial=renderScan.rows.filter(r=>r.classification==='VERIFY_REQUIRED');
  ...
  try{
   ...
   notifyInfo('Legacy recovery preview готов',`Проверено: ${preview.total} • новых поколений: ${preview.newGenerations.length} • уже загружены точно: ${preview.uploadedExact.length} • дубликаты: ${preview.duplicates.length} • ручная проверка: ${preview.verifyRequired.length} • invalid/errors: ${preview.invalid.length+hashErrors}. YouTube upload: 0.`)
 ...
 function confirmLegacyNewGenerations(){
  ...
  for(const decision of legacyRecoveryPreview.newGenerations){
   ...
   safe.push({...row,classification:'NEW_GENERATION',reason:'BULK_LEGACY_RECOVERY_CONFIRMED'});
  }
  ...
  const ok=window.confirm(`Подтвердить ${safe.length} новых физических поколений? Старые YouTube ID и uploadHistory сохранятся. Загрузка на YouTube НЕ начнётся.`);
  if(!ok)return;
 ...
 return <>
  ...
  <div className="publishStepGrid">
   <section className="panel publishStep"><div className="panelHead"><div><small>01 • VIDEO</small><h3>Готовые видео</h3><p>{newCount} новых · {uploadedCount} на YouTube · {processingCount} обрабатываются · {verifyCount} требуют проверки · {errorCount} ошибок</p></div><span>{draft.selectedIds.length} выбрано</span></div><div className="publishToolbar"><button className={videoFilter==='new'?'active':''} onClick={()=>setVideoFilter('new')}>Новые {newCount}</button><button className={videoFilter==='youtube'?'active':''} onClick={()=>setVideoFilter('youtube')}>На YouTube {uploadedCount}</button><button className={videoFilter==='processing'?'active':''} onClick={()=>setVideoFilter('processing')}>Обрабатываются {processingCount}</button><button className={videoFilter==='verify'?'active':''} onClick={()=>setVideoFilter('verify')}>Нужна проверка {verifyCount}</button><button className={videoFilter==='errors'?'active':''} onClick={()=>setVideoFilter('errors')}>Ошибки {errorCount}</button><button className={videoFilter==='all'?'active':''} onClick={()=>setVideoFilter('all')}>Все {stateCounts.ALL}</button></div><div className="publishToolbar"><button disabled={!channelJobs.some(j=>selectableJobIds.has(j.id))} onClick={()=>setDraftPatch({selectedIds:channelJobs.filter(j=>selectableJobIds.has(j.id)).map(j=>j.id)})}>Выбрать все</button><button onClick={()=>setDraftPatch({selectedIds:[]})}>Снять выбор</button><button disabled={busy||renderScanBusy} onClick={()=>void scanRenderFolder()}>{renderScanBusy?'Сканирую…':'↻ Просканировать папку рендера'}</button><button disabled={busy||renderScanBusy} onClick={()=>void autoDiscoverChannelRenderFolder()}>Найти автоматически</button><button disabled={busy||renderScanBusy} onClick={()=>void chooseChannelRenderFolder()}>Выбрать папку</button>{verifyCount>0&&<button disabled={busy} onClick={()=>void reconcileExistingUploads()}>↻ Проверить YouTube ID ({verifyCount})</button>}<button disabled={busy||!selected.some(j=>j.status!=='UPLOADING')} onClick={()=>setRemoveRequest({ids:selected.filter(j=>j.status!=='UPLOADING').map(j=>j.id),label:`Убрать из списка: ${selected.filter(j=>j.status!=='UPLOADING').length}`})}>Убрать выбранные</button>{uploadedIds.length>0&&<button className="danger" disabled={busy} onClick={()=>setRemoveRequest({ids:uploadedIds,label:`Очистить загруженные: ${uploadedIds.length}`})}>Очистить загруженные</button>}</div>{!channelRenderFolder&&<div className="publishCheck warn"><b>Папка: НЕ НАСТРОЕНА</b><span>Папка рендера для {channel?.name||'текущего канала'} не настроена. Глобальный Workspace НЕ будет сканироваться.</span><button onClick={()=>void autoDiscoverChannelRenderFolder()}>Найти автоматически</button><button onClick={()=>void chooseChannelRenderFolder()}>Выбрать папку</button></div>}{channelRenderFolder&&!renderScan&&<div className="publishCheck good"><b>Папка: CHANNEL-SPECIFIC ✓</b><span>{channelRenderFolder} • YouTube quota: 0</span></div>}{renderScan&&<div className="publishCheck good"><b>Папка канала: {renderScan.result.root}</b><span>Папка: CHANNEL-SPECIFIC ✓ • найдено: {renderScan.summary.TOTAL_CLASSIFIED_FILES} • совпало с сохранённым fingerprint: {renderScan.summary.UPLOADED_LOCAL_COPY} • новые: {renderScan.summary.NEW_CANDIDATE+renderScan.summary.NEW_GENERATION} • новые версии файла: {renderScan.summary.NEW_GENERATION} • проверить: {renderScan.summary.VERIFY_REQUIRED} • неоднозначно: {renderScan.summary.AMBIGUOUS} • duplicate: {renderScan.summary.DUPLICATE_LOCAL} • invalid: {renderScan.summary.INVALID} • YouTube quota: 0{renderScan.result.truncated?' • SCAN_TRUNCATED':''}</span>{(renderScan.summary.NEW_CANDIDATE+renderScan.summary.NEW_GENERATION)>0&&<button onClick={addScannedRenderCandidates}>Добавить новые ({renderScan.summary.NEW_CANDIDATE+renderScan.summary.NEW_GENERATION})</button>}{renderScan.summary.VERIFY_REQUIRED>0&&<button disabled={busy||renderScanBusy} onClick={()=>void bulkReconcileLegacyRenderRows()}>Проверить и восстановить {renderScan.summary.VERIFY_REQUIRED} файлов</button>}{legacyRecoveryPreview&&<div className="publisherNotice"><b>Предпросмотр legacy recovery</b><p>Файлов: <b>{legacyRecoveryPreview.total}</b> • новых поколений: <b>{legacyRecoveryPreview.newGenerations.length}</b> • уже загружены точно: <b>{legacyRecoveryPreview.uploadedExact.length}</b> • дубликаты: <b>{legacyRecoveryPreview.duplicates.length}</b> • ручная проверка: <b>{legacyRecoveryPreview.verifyRequired.length}</b> • invalid: <b>{legacyRecoveryPreview.invalid.length}</b>.</p><p>YouTube upload: <b>0</b>. Старые YouTube ID и uploadHistory не изменены.</p><button disabled={busy||renderScanBusy||legacyRecoveryPreview.newGenerations.length===0} onClick={confirmLegacyNewGenerations}>Подтвердить новые поколения ({legacyRecoveryPreview.newGenerations.length})</button></div>}{renderScan.importReport&&<details><summary>Результат добавления</summary><p>Запрошено: {renderScan.importReport.requested} • добавлено: {renderScan.importReport.added} • пропущено: {renderScan.importReport.skipped} • уже известно: {renderScan.importReport.alreadyKnown} • ошибок: {renderScan.importReport.errors}</p>{renderScan.importReport.details.map((x,i)=><p key={i}>{x}</p>)}</details>}<details><summary>Все физические файлы канала ({renderScan.rows.length})</summary>{renderScan.rows.map(row=>{const stateLabel=row.classification==='UPLOADED_LOCAL_COPY'?'Уже загружено — файл совпадает с сохранённым fingerprint':row.classification==='NEW_GENERATION'?'Новая версия файла':row.classification==='NEW_CANDIDATE'?'Новый физический файл':row.classification==='VERIFY_REQUIRED'?'Нужно подтвердить — identity текущего файла не доказана':row.reason;return <div key={row.file.path} className="renderIdentityEvidence"><p><b>{row.file.name}</b> • Sequence {row.sequence??'—'} • {(row.file.size/1024/1024).toFixed(1)} MB • {stateLabel}</p><details><summary>Показать доказательство статуса</summary><p><b>Current file</b></p><p className="mono">Path: {row.file.path}</p><p>Size: {row.currentFileSize??row.file.size} bytes</p><p className="mono">Fingerprint: {row.currentFingerprint||'НЕ ВЫЧИСЛЕН'}</p><p>Modified: {row.file.modifiedAt?new Date(row.file.modifiedAt).toLocaleString('ru-RU'):'—'}</p><p><b>Matched historical upload</b></p><p>Job ID: {row.historyJobId||row.matchedJobId||'—'}</p><p>YouTube ID: {row.youtubeVideoId||'—'}</p><p>Stored size: {row.historyFileSize??'—'}</p><p className="mono">Stored fingerprint: {row.historyFingerprint||'—'}</p><p>Uploaded: {row.historyUploadedAt?new Date(row.historyUploadedAt).toLocaleString('ru-RU'):'—'}</p><p>Classification: <b>{row.classification}</b> • {row.reason}</p><p>YouTube API requests: 0</p></details>{row.classification==='VERIFY_REQUIRED'&&<><button className="mini" disabled={renderScanBusy||busy} onClick={()=>void recheckRenderIdentity(row)}>Проверить файл</button><button className="mini" disabled={renderScanBusy||busy} onClick={()=>void treatVerifyRowAsNewGeneration(row)}>Считать текущий файл новой версией</button></>}</div>})}</details>{recoveryJobs.length>0&&<details open><summary>Неверная привязка локального файла ({recoveryJobs.length})</summary>{recoveryJobs.map(j=><div key={j.id} className="publishVideoRow"><span><b>VIDEO_{String(j.number).padStart(3,'0')} • CROSS_CHANNEL_SCAN_RECOVERY_REQUIRED</b><small>Current channel: {channel?.name} • Physical file: {j.finalPath}</small></span><button className="mini" onClick={()=>removeWrongScanJob(j)}>Убрать ошибочную запись из публикации</button><button className="mini" onClick={()=>rebindWrongScanJob(j)}>Перепривязать к правильному каналу</button></div>)}</details>}</div>}<div className="publishVideoRows">{channelJobs.length?channelJobs.map(j=>{const st=stateOf(j),proof=uploadForJob(j.id),videoId=j.youtubeVideoId||proof?.youtubeVideoId,recovery=recoveryJobIds.has(j.id),fresh=selectableJobIds.has(j.id),disabledReason=recovery?'Старая запись сканирования — требуется проверка':st==='READY'?'Уже загружено на YouTube':st==='UPLOAD_ACCEPTED'?'YouTube уже принял видео':st==='YOUTUBE_PROCESSING'?'Видео обрабатывается YouTube':st==='VERIFY_REQUIRED'?'Требуется проверка перед загрузкой':st==='REMOTE_MISSING'?'Связь YouTube требует проверки':st==='UPLOAD_FAILED'||st==='PROCESSING_FAILED'||st==='REJECTED'?'Ошибка — откройте действия':!j.finalPath?'Файл не найден':'Недоступно для новой загрузки',label=recovery?'НЕВЕРНАЯ ПРИВЯЗКА':st==='NEW'?'НОВОЕ':st==='UPLOAD_ACCEPTED'?'НА YOUTUBE':st==='READY'?'READY':st==='YOUTUBE_PROCESSING'?'ОБРАБОТКА YOUTUBE':st==='VERIFY_REQUIRED'?'НУЖНА ПРОВЕРКА':st==='REMOTE_MISSING'?'YOUTUBE: НЕ НАЙДЕНО':st;return <div key={j.id} className={`publishVideoRow ${draft.selectedIds.includes(j.id)?'selected':''} ${!fresh?'uploaded':''}`}><label className="publishVideoSelect"><input type="checkbox" disabled={!fresh||busy} checked={fresh&&draft.selectedIds.includes(j.id)} onChange={()=>fresh&&setDraftPatch({selectedIds:draft.selectedIds.includes(j.id)?draft.selectedIds.filter(x=>x!==j.id):[...draft.selectedIds,j.id]})}/><span><b>VIDEO_{String(j.number).padStart(3,'0')}</b><small>{baseName(j.finalPath||'')} • {label}{videoId?` • YouTube ID: ${videoId}`:''}{proof?.uploadedAt?` • ${new Date(proof.uploadedAt).toLocaleDateString('ru-RU')}`:''}{!fresh?` • ${disabledReason}`:''}</small></span>{j.uploadProgress!=null&&j.status==='UPLOADING'&&<em>{j.uploadProgress.toFixed(0)}%</em>}</label>{videoId&&st!=='READY'&&<button className="mini" disabled={busy} onClick={()=>void reconcileExistingUploads([j.id])}>Проверить</button>}{st==='REMOTE_MISSING'&&<button className="mini" disabled={busy} onClick={()=>clearRemoteMissing(j)}>Очистить связь</button>}<button className="mini" disabled={busy||j.status==='UPLOADING'} onClick={()=>setRemoveRequest({ids:[j.id],label:`VIDEO_${String(j.number).padStart(3,'0')}`})}>Действия</button></div>}):<p>{videoFilter==='youtube'?'Подтверждённых видео YouTube в локальном списке нет.':videoFilter==='new'?'В VYRON нет зарегистрированных готовых видео. Просканируйте папку рендера.':'Для выбранного фильтра видео нет.'}</p>}</div></section>
