...
function UploadCenterModal(){
 ...
 return <><div className="modalBackdrop uploadCenterBackdrop" onMouseDown={closeUploadCenter}><section className="uploadCenterModal" onMouseDown={e=>e.stopPropagation()}>
  <div className="panelHead uploadCenterHead"><div><small>ЦЕНТР ЗАГРУЗОК</small><h2>Загрузки YouTube</h2><p>Очередь, текущая передача и история загрузок.</p></div><div className="uploadCenterHeadStats"><span><small>В очереди</small><b>{queue.queued.length}</b></span><span><small>Загружаются</small><b>{activeCount}</b></span><span><small>Успешно</small><b>{succeeded.length}</b></span><span><small>Ошибки</small><b>{failed.length}</b></span><button onClick={closeUploadCenter}>×</button></div></div>
  ...
  {persisted.length>0&&<section className="panel uploadHistoryPanel"><div className="panelHead"><div><small>ИСТОРИЯ ЗАГРУЗОК</small><h3>Последние загрузки</h3></div><div className="headerActions"><button onClick={()=>{useApp.getState().setPage('youtube');closeUploadCenter();setTimeout(()=>window.dispatchEvent(new Event('vyron:youtube-history')),0)}}>Вся история</button><button disabled={sourceScanBusy} onClick={()=>void scanLocalSources()}>{sourceScanBusy?'Проверяю…':'Проверить локальные файлы'}</button>{cleanupCandidates.length>0&&<button onClick={()=>setCleanupOpen(true)}>Очистить безопасные • {cleanupCandidates.length}</button>}</div></div>{persisted.map(x=><article className="uploadHistoryRow" key={x.id}><div className="uploadHistoryIdentity"><b>{x.titleAtUpload||x.originalFilename}</b><small>{new Date(x.uploadedAt).toLocaleString('ru-RU')}</small><small>YouTube: {x.remoteExists===false?'Видео недоступно':x.processingState||'Принято'} • Локальный файл: {effectiveSourceLifecycle(x)}</small></div><div className="uploadHistoryState"><span>{x.trashedAt?'Перемещён в Корзину':x.sourceLifecycle||'Статус неизвестен'}</span><details><summary>Технические сведения</summary><small>YouTube ID: {x.youtubeVideoId}</small></details></div></article>)}</section>}
 </section></div>
