...
function UploadCenterModal(){
 ...
 {queue.queued.map((entry,index)=><article className="uploadCenterRow queued" key={entry.queueId}><div className="uploadIdentity"><small>{entry.spec.channelName}</small><b>{videoName(entry.spec)}</b><span>{entry.spec.title||entry.spec.projectId}</span></div><div className="queuedState"><b>QUEUED</b><span>Позиция в очереди: {index+1}</span><button onClick={()=>removeQueuedUpload(entry.spec.jobId)}>Убрать из очереди</button></div><div className="uploadTiming"><span><small>Отправлено в очередь</small><b>{localClock(entry.spec.submittedAt)}</b></span><span><small>Progress</small><b>после старта transfer</b></span></div></article>)}
 {!activeCount&&!queue.queued.length&&<div className="uploadCenterEmpty"><strong>✓</strong><div><b>Активных загрузок нет</b><span>Historical UPLOADING status сам по себе не считается активным transfer.</span></div></div>}
 </div></section></div>;
}
