...
export type KeychainDiagnostic={ok:boolean;status:string;service?:string;backend?:string;canonicalVisibleAccounts?:number;legacyVisibleAccounts?:number;secretValuesIncluded?:boolean};
export type EndlumeDiagnostic={ok:boolean;status:'READY'|'PATH_MISSING'|'FILE_NOT_FOUND'|'NOT_EXECUTABLE'|'INBOX_NOT_WRITABLE'|'VERSION_UNKNOWN'|'HANDSHAKE_FAILED';detail?:string;path?:string;inbox?:string;versionStatus?:string;handshakeStatus?:string;destructiveActions?:boolean};
export type UpdaterManifestDiagnostic={ok:boolean;status:string;endpoint:string;httpStatus?:number;endpointReachable?:boolean;jsonParsed?:boolean;schemaValid?:boolean;platformKey?:string;platformPresent?:boolean;version?:string;semverValid?:boolean;downloadUrl?:string;signaturePresent?:boolean;currentVersion?:string;comparison?:string;errors?:string[];installerDownloaded?:boolean;youtubeApiRequests?:number;detail?:string};
...
export type OAuthRecoveryDiagnostic={appVersion:string;bundleId:string;channelId?:string;currentProfileUuid:string;legacyService:string;canonicalService:string;canonicalRefreshPresent:boolean;legacyRefreshPresent:boolean;migrationState:string;credentialState:'CANONICAL_READY'|'LEGACY_RECONNECT_REQUIRED'|'MISSING';secretValuesIncluded:false;youtubeApiRequests:0;keychainSecretReads:0;path:string};
...
export const api={
  ...
  endlumeDiagnostics:(endlumePath:string)=>invoke<EndlumeDiagnostic>('endlume_diagnostics',{endlumePath}),
  updaterManifestDiagnostics:(endpoint:string)=>invoke<UpdaterManifestDiagnostic>('updater_manifest_diagnostics',{endpoint}),
  ...
  chooseEndlume:async()=>{const r=await open({directory:false,multiple:false,title:'Выберите ENDLUME'});return typeof r==='string'?r:null},
