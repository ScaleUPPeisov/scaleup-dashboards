...
export type OAuthRecoveryDiagnostic={appVersion:string;bundleId:string;channelId?:string;currentProfileUuid:string;legacyService:string;canonicalService:string;canonicalRefreshPresent:boolean;legacyRefreshPresent:boolean;migrationState:string;credentialState:'CANONICAL_READY'|'LEGACY_AUTH_REQUIRED'|'MISSING';secretValuesIncluded:false;youtubeApiRequests:0;keychainSecretReads:0;path:string};
export type YoutubeKeychainMigrationDiagnostic={version:number;legacyService:string;canonicalService:string;profileStates:Record<string,string>;globalClientSecretState:string;migratedProfiles:number;failedProfiles:number;profileMigrationAttempts:number;profileMigrationSuccesses:number;profileMigrationFailures:number;accessTokenMemoryHits:number;secretValuesIncluded:false};
