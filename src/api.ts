...
export type OAuthCredentialState='READY'|'CONNECTED'|'CANONICAL_PRESENT_UNVERIFIED'|'RECOVERABLE_KEYCHAIN_BLOCKED'|'KEYCHAIN_BLOCKED'|'RECONNECT_REQUIRED'|'MISSING'|'WRONG_CHANNEL'|'FAILED'|'NOT_CHECKED';
...
export const api={
  ...
  youtubeOauthVaultRecover:()=>invoke<{status:string;recovered:boolean;localStorage?:{root:string;vaultPath:string;keyPath:string;localKeyPresent:boolean;localVaultPresent:boolean;localVaultReadable:boolean;legacyVaultPresent:boolean;backupCount:number;keychainRequiredForNormalStartup:boolean};googleBrowserLaunches:number;youtubeApiRequests:number}>('youtube_oauth_vault_recover'),
  youtubeOauthLocalStorageStatus:()=>invoke<{root:string;authPath:string;statePath:string;logsPath:string;authBackupPath:string;vaultPath:string;keyPath:string;localKeyPresent:boolean;localVaultPresent:boolean;localVaultReadable:boolean;legacyVaultPresent:boolean;backupCount:number;keychainRequiredForNormalStartup:boolean;secretValuesIncluded:boolean}>('youtube_oauth_local_storage_status'),
