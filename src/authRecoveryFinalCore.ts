...
export function buildFinalRecoveryRows(channels:RecoveryChannelLike[],profiles:RecoveryProfileLike[],credentialStates:RecoveryCredentialStateLike[],transient:Record<string,RecoveryTransient>={}){
 ...
 return profiles.map(profile=>{
  ...
  if(state.clientSecretState==='MISSING')return{profileId:profile.id,profile,channels:mapped,expectedChannelId,status:'CLIENT SECRET REQUIRED',detail:'Глобальный OAuth Client VYRON не готов: client_secret отсутствует. Настройте credentials.json один раз для приложения; отдельный файл для каждого канала не нужен.',stale,duplicate,conflict} satisfies FinalRecoveryRow;
...
export function reconnectFailure(error:unknown):{status:'CLIENT SECRET REQUIRED'|'WRONG CHANNEL'|'FAILED';message:string}{const raw=String(error??'');const low=raw.toLowerCase();if(low.includes('oauth_client_secret_required')||low.includes('oauth_client_secret_reimport_required')||low.includes('client_secret is missing'))return{status:'CLIENT SECRET REQUIRED',message:'OAuth Client не настроен: импортируйте credentials.json, соответствующий client_id этого профиля.'};if(low.includes('wrong_channel'))return{status:'WRONG CHANNEL',message:raw.replace(/^.*WRONG_CHANNEL:\s*/i,'')||'Авторизован другой YouTube канал.'};if(low.includes('oauth_refresh_token_required')||low.includes('google не вернул refresh token'))return{status:'FAILED',message:'Google не вернул refresh token. Повторите подключение.'};return{status:'FAILED',message:raw.replace(/^Error:\s*/i,'')||'Подключение не завершено.'}}
