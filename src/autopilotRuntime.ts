...
function logError(summary:AutopilotSummary,label:string,e:unknown){summary.errors++;const h=humanizeError(e,label.toLocaleLowerCase('ru-RU').includes('endlume')?'endlume':'storage');const msg=`${label}: ${h.message}`;summary.notes.push(msg);useApp.getState().log(msg,'error')}
