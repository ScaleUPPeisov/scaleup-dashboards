...
export type ErrorHistoryMeta={errorCode?:string;videoId?:string;projectId?:string;filePath?:string;stage?:ErrorStage;profileId?:string;channelId?:string;currentVersion?:string;targetVersion?:string};
export type ErrorHistoryItem={id:string;title:string;message:string;technicalDetail?:string;errorCode?:string;videoId?:string;projectId?:string;filePath?:string;stage?:ErrorStage;profileId?:string;channelId?:string;currentVersion?:string;targetVersion?:string;createdAt:number;resolvedAt?:number;resolvedBy?:string};
...
export function appendErrorHistory(title:string,message='',technicalDetail='',meta:ErrorHistoryMeta={}){const row:ErrorHistoryItem={id:crypto.randomUUID(),title,message,technicalDetail:technicalDetail||undefined,errorCode:meta.errorCode,videoId:meta.videoId,projectId:meta.projectId,filePath:meta.filePath,stage:meta.stage,profileId:meta.profileId,channelId:meta.channelId,currentVersion:meta.currentVersion,targetVersion:meta.targetVersion,createdAt:Date.now()};save([...load(),row]);return row}
export function errorAttributionParts(row:Pick<ErrorHistoryItem,'channelId'|'profileId'|'projectId'|'videoId'|'filePath'>){return [row.channelId&&`channel:${row.channelId}`,row.projectId&&`project:${row.projectId}`,row.videoId&&`video:${row.videoId}`,row.filePath&&`file:${row.filePath}`,row.profileId&&`profile:${row.profileId}`].filter((x):x is string=>Boolean(x))}
