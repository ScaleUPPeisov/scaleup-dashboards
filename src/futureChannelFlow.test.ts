...
describe('VYRON future-channel integration',()=>{
 ...
 it('exposes explicit future-channel creation in Channels',()=>{const s=readFileSync('src/ChannelsOS.tsx','utf8');expect(s).toContain('+ Будущий канал');expect(s).toContain('Добавить канал без YouTube');expect(s).toContain("isFutureChannel(c)?'Будущий канал':'YouTube не подключён'")});
