...
describe('VYRON passive Keychain architecture',()=>{
 ...
 it('Google config status is metadata-only',()=>{const x=body(rust,'pub fn youtube_google_config_status','fn validate_imported_client_id');expect(x).toContain('load_google_config_metadata');expect(x).not.toMatch(/get_secret|hydrate_google|load_or_migrate/)});
 it('Google project diagnostics stays zero API and zero Keychain',()=>{const x=body(rust,'pub fn youtube_google_project_diagnostic','fn validate_imported_client_id');for(const token of ['get_secret','valid_access_token','reqwest','refresh_token','access_token','client_secret'])expect(x).not.toContain(token)});
