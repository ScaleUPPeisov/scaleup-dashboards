...
export type ShortScheduleOptions={startDate:string;times:string[];count:number;occupied?:string[];now?:Date};
...
export function shortsScheduleSlots(opts:ShortScheduleOptions){return publisherUnifiedChannelSlots({...opts,now:opts.now||new Date()})}
