...
describe('Storage Lifecycle + Duplicate Upload Guard',()=>{
 ...
 it('UI defaults to New and only canonical NEW items are normally selectable',()=>{expect(pub).toContain("type VideoFilter='new'|'youtube'|'processing'|'verify'|'errors'|'all'");expect(pub).toContain("useState<VideoFilter>('new')");expect(pub).toContain('Новые {newCount}');expect(pub).toContain('На YouTube {uploadedCount}');expect(pub).toContain('Нужна проверка {verifyCount}');expect(pub).toContain('Все {stateCounts.ALL}');expect(pub).toContain("const selectableJobs=useMemo(()=>allChannelJobs.filter(j=>uploadStateById.get(j.id)==='NEW')");expect(pub).toContain("disabled={!fresh||busy}");expect(pub).toContain('Расширенное восстановление дубля')});
