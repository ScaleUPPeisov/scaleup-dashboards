...
describe('Storage Lifecycle + Duplicate Upload Guard',()=>{
 ...
 it('failed/verification-failed upload cannot persist success proof',()=>{const branch=queueRuntime.indexOf('if(uploaded.verified===false)');const persist=queueRuntime.indexOf('const nextHistory=recordVerifiedUpload',branch);expect(branch).toBeGreaterThanOrEqual(0);expect(persist).toBeGreaterThan(branch)});
