...
export function successfulUploadForHash(history:UploadHistoryRecord[],sha256:string){const h=sha256.trim().toLowerCase();return history.slice().reverse().find(x=>x.status==='UPLOADED'&&Boolean(x.youtubeVideoId)&&!x.staleLinkClearedAt&&x.sha256.toLowerCase()===h)}
