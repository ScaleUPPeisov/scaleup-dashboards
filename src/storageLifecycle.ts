...
export function successfulUploadForHash(history:UploadHistoryRecord[],sha256:string,channelId?:string){const h=sha256.trim().toLowerCase();return history.slice().reverse().find(x=>x.status==='UPLOADED'&&Boolean(x.youtubeVideoId)&&!x.staleLinkClearedAt&&(!channelId||x.channelId===channelId)&&x.sha256.toLowerCase()===h)}
export function duplicateUploadIds(jobs:VideoJob[],fingerprints:Record<string,{sha256:string}>,history:UploadHistoryRecord[],allowOverrideIds:Set<string>=new Set()){return jobs.filter(j=>{if(allowOverrideIds.has(j.id))return false;const fp=fingerprints[j.id];return Boolean(fp&&successfulUploadForHash(history,fp.sha256,j.channelId))}).map(j=>j.id)}
...
export function cleanupEligibleUpload(x:UploadHistoryRecord,job?:VideoJob){if(x.status!=='UPLOADED'||x.processingState!=='READY'||x.remoteExists===false||!x.youtubeVideoId?.trim()||!x.profileId?.trim()||!x.localFilePath?.trim()||!x.sha256?.trim()||Boolean(x.trashedAt)||x.sourceLifecycle!=='PRESENT')return false;if(job){if(job.id!==x.jobId||job.finalPath!==x.localFilePath)return false;if(job.uploadFingerprint&&job.uploadFingerprint.toLowerCase()!==x.sha256.toLowerCase())return false}return true}
