...
describe('VYRON remote in-app update product contract',()=>{
 ...
 it('renders a persistent updater indicator in Sidebar and removes floating UpdateNotice',()=>{expect(app).toContain('<UpdaterSidebar/>');expect(app).toContain('🔔 Доступно обновление');expect(app).toContain('ОБНОВИТЬ');expect(app).not.toContain('function UpdateNotice(')});
