...
describe('shared remote updater lifecycle',()=>{
 ...
 it('does not install or restart while critical jobs exist',async()=>{const c=candidate();mockApi.checkUpdate.mockResolvedValue(c);await useUpdaterRuntime.getState().check();await useUpdaterRuntime.getState().download();const ok=await useUpdaterRuntime.getState().installAndRestart([{kind:'SHORTS',label:'Shorts',count:3}]);expect(ok).toBe(false);expect(c.install).not.toHaveBeenCalled();expect(c.restart).not.toHaveBeenCalled();expect(useUpdaterRuntime.getState().blockers[0]?.count).toBe(3)});
