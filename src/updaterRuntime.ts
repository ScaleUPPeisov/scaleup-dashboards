...
export type UpdaterRuntimeState={
  currentVersion:string;latestVersion:string;currentBuildRevision:number;latestBuildRevision:number;updateChannel:string;commit:string;artifactSha256?:string;status:RuntimeUpdaterStatus;progress:number;downloadedBytes:number;totalBytes:number;
  ...
  installAndRestart:(blockers?:UpdaterBlocker[])=>Promise<boolean>;markUpdated:(version:string,buildRevision?:number)=>void;clearBlockers:()=>void;
};
...
export const useUpdaterRuntime=create<UpdaterRuntimeState>((set,get)=>({
  currentVersion:'',latestVersion:'',currentBuildRevision:0,latestBuildRevision:0,updateChannel:'stable',commit:'',status:'UP_TO_DATE',progress:0,downloadedBytes:0,totalBytes:0,notes:'',endpoint:'',versionComparison:'',blockers:[],hasChecked:false,consecutiveCheckFailures:0,preflight:undefined,
  bootstrapVersion:async()=>{if(get().currentVersion)return;try{const id=await api.updaterRuntimeIdentity();set({currentVersion:id.productVersion,latestVersion:get().latestVersion||id.productVersion,currentBuildRevision:id.buildRevision,latestBuildRevision:get().latestBuildRevision||id.buildRevision,updateChannel:id.channel,commit:id.commit})}catch(error){const code=recordFailure('check',error,'','');set({status:'ERROR',errorCode:code,errorMessage:String(error),hasChecked:true,lastCheckedAt:Date.now()})}},
  check:async(options={})=>{
    ...
    checkPromise=(async()=>{
      ...
      try{
        ...
        if(result.none){
          candidate=undefined;
          set({currentVersion:result.current||prior.currentVersion,latestVersion:result.latest||result.current||prior.currentVersion,currentBuildRevision:result.currentBuildRevision??prior.currentBuildRevision,latestBuildRevision:result.buildRevision??result.currentBuildRevision??prior.currentBuildRevision,updateChannel:result.channel||prior.updateChannel,status:'UP_TO_DATE',endpoint:result.endpoint||'',versionComparison:result.versionComparison||'',lastCheckedAt:checkedAt,lastCheckAttemptAt:attemptAt,nextAutomaticCheckAt:checkedAt+UPDATER_AUTO_INTERVAL_MS,consecutiveCheckFailures:0,hasChecked:true,progress:0,downloadedBytes:0,totalBytes:0,notes:''});
          return;
        }
        candidate=result;
        set({currentVersion:result.current||prior.currentVersion,latestVersion:result.latest||result.version||'',currentBuildRevision:result.currentBuildRevision??prior.currentBuildRevision,latestBuildRevision:result.buildRevision??prior.latestBuildRevision,updateChannel:result.channel||prior.updateChannel,artifactSha256:result.artifactSha256,status:'AVAILABLE',notes:result.body||'',releaseDate:result.date,endpoint:result.endpoint||'',versionComparison:result.versionComparison||'',lastCheckedAt:checkedAt,lastCheckAttemptAt:attemptAt,nextAutomaticCheckAt:checkedAt+UPDATER_AUTO_INTERVAL_MS,consecutiveCheckFailures:0,hasChecked:true,progress:0,downloadedBytes:0,totalBytes:0});
      }catch(error){
