...
async function executeUpload(spec:ImmutableUploadJob){
 ...
 try{
  ...
  if(uploaded.verified===false){
    phase='REMOTE_VERIFY';
    const uncertainHistory=recordVerifiedUpload(acceptedState.uploadHistory,{jobId:spec.jobId,channelId:spec.channelId,profileId:spec.profileId,youtubeChannelId:spec.youtubeChannelId,youtubeVideoId:uploaded.videoId,localFilePath:spec.filePath,originalFilename:baseName(spec.filePath),projectId:spec.projectId||acceptedProject?.[1].projectId,sourceProjectPath:acceptedProject?.[1].projectPath,batchId,titleAtUpload:spec.title,uploadedAt:acceptedAt,fileSize:spec.fileSize,sha256:spec.fingerprint,fingerprintProofSource:'UPLOAD_TIME',fingerprintCapturedAt:startedAt,sourceGenerationKeyAtUpload:`${spec.channelId}:${spec.fingerprint}:${spec.fileSize}`,uploadOperationId:operationId,proofSchemaVersion:1,publishAt:spec.publishAt,overrideDuplicate:spec.allowDuplicate,sourceLifecycle:'PRESENT',processingState:'PROCESSING_UNKNOWN',processingCheckedAt:acceptedAt,processingError:uploaded.verificationError});
