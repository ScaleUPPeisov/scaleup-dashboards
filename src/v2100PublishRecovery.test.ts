...
describe('VYRON 2.1 publish recovery',()=>{
 ...
 it('wires real details and clearable Error Center',()=>{const p=readFileSync('src/PublisherOS.tsx','utf8'),a=readFileSync('src/App.tsx','utf8'),n=readFileSync('src/notificationCenter.ts','utf8');expect(p).toContain('syncScheduleFromYoutube');expect(p).toContain('pastSchedule');expect(p).toContain('Получить актуальное расписание канала');expect(p).toContain('technicalDetail:h.detail');expect(a).toContain('Очистить активные');expect(a).toContain('Очистить историю');expect(a).toContain('Убрать ошибку');expect(a).toContain('Технические детали');expect(n).toContain('technicalDetail?:string')});
});
