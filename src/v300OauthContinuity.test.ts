...
describe('VYRON 3.0.0 OAuth/client-secret continuity contracts',()=>{
 ...
 it('allows explicit one-time global credentials repair while Keychain is blocked',()=>{expect(accounts).toContain('Восстановить через credentials.json');expect(accounts).not.toContain('{!oauthKeychainBlocked&&<button');});
