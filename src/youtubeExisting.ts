...
function isoForCalendarTime(date:string,time:string,offsetMinutes?:number){
  ...
  const krat=new Date(Date.UTC(y,m-1,d,hh,mm)-7*60*60000);
  return Number.isNaN(krat.getTime())?undefined:krat.toISOString();
}

export function buildExistingScheduleFromLocal(videos:YoutubeExistingVideo[],startLocal:string,cadenceDays:number,meta:(ImportedMetadata|undefined)[]=[]){
