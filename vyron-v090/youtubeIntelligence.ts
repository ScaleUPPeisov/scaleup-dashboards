...
export async function refreshYoutubeIntelligence(force=false){
 if(running)return;running=true;const s=useApp.getState(),errors:string[]=[];try{for(const channel of s.channels.filter(c=>c.enabled&&c.youtubeProfileId)){try{await refreshChannelAnalytics(channel.id,28,force)}catch(e){errors.push(`${channel.name}: ${String(e)}`)}for(const c of useApp.getState().competitors.filter(c=>c.channelId===channel.id)){try{await refreshCompetitor(c.id,force)}catch(e){errors.push(`${c.name}: ${String(e)}`)}}}if(errors.length)useApp.getState().log(`YouTube Intelligence: ${errors.slice(0,5).join(' • ')}`,'warn');else if(force)useApp.getState().log('YouTube Intelligence обновлён')}finally{running=false}
}
