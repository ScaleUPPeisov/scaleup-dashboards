  ...
  {channels.length===0?<div className="empty"><b>Каналов пока нет</b><p>Добавь локальный канал или подключи реальный аккаунт во вкладке YouTube.</p></div>:<div className="channelOSList">{channels.map(c=>{
   ...
   return <article className={`channelOSCard ${editing===c.id?'editing':''}`} key={c.id}>
    ...
    {editing===c.id&&<div className="editGrid channelEdit">
     ...
     <label className="wide">Формулы названий<textarea value={c.seo.titlePatterns.join('\n')} onChange={e=>updateChannel(c.id,{seo:{...c.seo,titlePatterns:e.target.value.split('\n').filter(Boolean)}})}/></label><label className="wide">Шаблон описания<textarea value={c.seo.descriptionTemplate} onChange={e=>updateChannel(c.id,{seo:{...c.seo,descriptionTemplate:e.target.value})}/></label><label className="wide">Теги<input value={c.seo.tags.join(', ')} onChange={e=>updateChannel(c.id,{seo:{...c.seo,tags:e.target.value.split(',').map(x=>x.trim()).filter(Boolean)}})}/></label>
     <div className="editActions"><button className="danger" onClick={()=>{if(window.confirm(`Удалить ${c.name}? Файлы на диске останутся.`)){removeChannel(c.id);setEditing(null)}}}>Удалить из VYRON</button><button className="primary" onClick={()=>setEditing(null)}>Готово</button></div>
    </div>}
   </article>
  })}</div>}
 </>
