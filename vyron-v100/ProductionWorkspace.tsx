...
export function ProductionWorkspace(){
 ...
 useEffect(()=>{if(!selected)return;if(state.selectedChannelId===selected.id&&state.byChannel[selected.id])return;const next={...state,selectedChannelId:selected.id,byChannel:{...state.byChannel,[selected.id]:state.byChannel[selected.id]||{project:month(),target:30,reviewed:false,updatedAt:new Date().toISOString()}}};setState(next);write(next)},[selected?.id]);
 const pref:selectedPref=selected?state.byChannel[selected.id]:undefined as any;
