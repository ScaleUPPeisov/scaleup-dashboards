...
import {buildYoutubeQuotaPlan,clearYoutubeQuotaGuard,loadYoutubeQuotaPlan,saveYoutubeQuotaPlan,setYoutubeQuotaLimit,subscribeYoutubeQuota,youtubeQuotaState,youtubeQuotaUsage} from './youtubeQuota';
...
export function QuotaMeter({compact=false,defaultChannels=100}:{compact?:boolean;defaultChannels?:number}){
 ...
 return <section className={`quotaMeter ${guard.blocked?'blocked':''} ${compact?'compact':''}`}>
  ...
  <div className="quotaMeterStats"><div><small>Использовано</small><b>{usage.used.toLocaleString('ru-RU')}</b><em>из {usage.limit.toLocaleString('ru-RU')}</em></div><div><small>Осталось</small><b>{remaining.toLocaleString('ru-RU')}</b><em>units</em></div><div><small>Сегодня можно</small><b>{plan.todayChannels}</b><em>каналов × {videos} видео</em></div><div><small>Сброс</small><b>00:00 PT</b><em>дневной пул</em></div></div>
