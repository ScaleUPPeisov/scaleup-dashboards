...
export const DEFAULT_SETTINGS:Settings={
  ...
  youtubeIntelligenceAutoRefresh:true,youtubeIntelligenceRefreshMin:30,
...
export const useApp=create<Store>((set,get)=>({
  ...
  hydrate:s=>set({...EMPTY_STATE,...s,version:7,channels:(s.channels||[]).map(normalizeChannel),jobs:(s.jobs||[]).map(normalizeJob),competitors:s.competitors||[],settings:{...DEFAULT_SETTINGS,...s.settings},logs:s.logs||[],booted:true}),
  ...
  toast:notice=>{set({notice});window.setTimeout(()=>{if(get().notice===notice)set({notice:undefined})},2800)}
}));
