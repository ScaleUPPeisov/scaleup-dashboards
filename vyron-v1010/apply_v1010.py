...
p=Path('src-tauri/Cargo.toml'); cargo=p.read_text(); must('version = "1.0.9"' in cargo,'expected Cargo 1.0.9'); cargo=cargo.replace('version = "1.0.9"','version = "1.0.10"',1); p.write_text(cargo)

# Native macOS updater bridge. Tauri updater creates TempDir under $TMPDIR and
# then renames app bundles. When $TMPDIR and VYRON.app are on different volumes,
# macOS returns EXDEV / Cross-device link (os error 18). Before installation we
# switch TMPDIR to a writable directory beside VYRON.app, but only when device
# ids actually differ. Signature verification/download/install remain Tauri's.
    ...
    #[cfg(target_os="macos")]
    {
        ...
        fs::create_dir_all(&local_tmp).map_err(|_|{
            if app.to_string_lossy().starts_with("/Volumes/"){
                ...
                "Updater не может создать временную папку рядом с VYRON.app. Проверь права на папку приложения.".to_string()
            }
        })?;
        ...
        // SAFETY: this command is called immediately before updater installation;
        // VYRON does not concurrently mutate process environment elsewhere.
...
if 'mod updater_bridge;' not in lib: lib='mod updater_bridge;\n'+lib
...
p=Path('src/api.ts'); api=p.read_text();
needle='await update.downloadAndInstall((event:any)=>{'
must(needle in api,'downloadAndInstall marker missing')
api=api.replace(needle,"await invoke<string>('prepare_updater_tempdir');\n      "+needle,1)
p.write_text(api)

# Keep package-lock version in sync when present.
...
print('VYRON 1.0.10 updater EXDEV hotfix applied')
