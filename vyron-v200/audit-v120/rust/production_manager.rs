...
fn execute_plan(app:Option<&AppHandle>,plan:&BuildPlan)->Result<BatchSummary,String>{
    ...
    let status=BatchStatus{batch_id:plan.batch_id.clone(),status:"Готово".into(),updated_at:Utc::now().to_rfc3339(),projects:manifest.projects.iter().map(|p|BatchProjectStatus{project_id:p.project_id.clone(),job_id:p.job_id.clone(),video_number:p.video_number,render_status:"Waiting".into(),output_file:None,duration:None,file_size:None,error:None,progress:None,started_at:None,handoff_status:None,handoff_sent_at:None,handoff_count:None}).collect()};
...
fn validate_manifest_projects(m:&BatchManifest,endlume_path:&str,selected:Option<&HashSet<String>>)->Validation{
