...
use std::{fs,io::{Read,Write,Seek},net::TcpListener,path::{Path,PathBuf},process::Command,time::{Duration,SystemTime,UNIX_EPOCH}};
...
pub async fn youtube_list_existing_videos(app:AppHandle,profile_id:String,max_results:Option<u32>)->Result<Value,String>{
 ...
 while ids.len()<limit{
  emit_youtube_api_request(&app,"playlistItems.list",None);let mut q=client.get("https://www.googleapis.com/youtube/v3/playlistItems").bearer_auth(&token).query(&[("part","contentDetails"),("playlistId",uploads),("maxResults","50")]);if let Some(ref t)=page{q=q.query(&[("pageToken",t.as_str())]);}
...
pub async fn youtube_update_existing_video(app:AppHandle,profile_id:String,video_id:String,title:String,description:String,tags:Vec<String>,publish_at:Option<String>,privacy_status:Option<String>,operation_id:Option<String>)->Result<Value,String>{
 ...
 if metadata_needed&&schedule_needed{
  ...
  emit_youtube_api_request(&app,"videos.update",operation_id.as_deref());let u=client.put("https://www.googleapis.com/youtube/v3/videos").bearer_auth(&token).query(&[("part","snippet,status")]).json(&body).send().await.map_err(|e|format!("YouTube combined update: {e}"))?;
  ...
  if ust.is_success(){metadata_accepted=true;schedule_accepted=true;response=uv}else{
   ...
   emit_youtube_api_request(&app,"videos.update",operation_id.as_deref());let mu=client.put("https://www.googleapis.com/youtube/v3/videos").bearer_auth(&token).query(&[("part","snippet")]).json(&sb).send().await.map_err(|e|format!("YouTube metadata fallback: {e}"))?;
