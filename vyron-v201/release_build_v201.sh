...
ROOT="$PWD"
WORK="$ROOT/.vyron-v201-release"
ASSETS='/tmp/vyron-v201-release-base'
...
Path('/tmp/v201-release-id').write_text(c['identifier'])
Path('/tmp/v201-release-product').write_text(c.get('productName',''))
Path('/tmp/v201-release-pub').write_text(c['plugins']['updater']['pubkey'])
Path('/tmp/v201-release-endpoints').write_text(json.dumps(c['plugins']['updater']['endpoints']))
PY
...
python3 "$ROOT/vyron-v201/apply_v201_quota_rollover_fix.py" .
python3 "$ROOT/vyron-v201/apply_v201_version.py" .
...
assert c['identifier']==Path('/tmp/v201-release-id').read_text()
assert c.get('productName','')==Path('/tmp/v201-release-product').read_text()
assert c['plugins']['updater']['pubkey']==Path('/tmp/v201-release-pub').read_text()
assert c['plugins']['updater']['pubkey'].strip()==os.environ['TAURI_UPDATER_PUBLIC_KEY'].strip(),'secret/public key mismatch'
assert json.dumps(c['plugins']['updater']['endpoints'])==Path('/tmp/v201-release-endpoints').read_text()
quota=Path('src/youtubeQuota.ts').read_text();meta=Path('src/MetadataPage.tsx').read_text();rust=Path('src-tauri/src/youtube.rs').read_text()
assert 'beginManualYoutubeQuotaProbe' in quota and 'shouldClearYoutubeQuotaGuard' in quota
assert "s.includes('quotaexceeded')" in quota
assert "YouTube API quota временно исчерпана. Сброс: 14:00." not in quota
assert 'beginManualYoutubeQuotaProbe()' in meta
assert 'playlistItems.list' in rust and 'nextPageToken' in rust and 'videos.list' in rust
assert 'search.list' not in rust
print('VYRON 2.0.1 signed-build source contracts: PASS')
PY
...
test "$(plutil -extract CFBundleIdentifier raw -o - "$APP/Contents/Info.plist")" = "$(cat /tmp/v201-release-id)"
test "$(plutil -extract CFBundleShortVersionString raw -o - "$APP/Contents/Info.plist")" = '2.0.1'
codesign --verify --deep --strict --verbose=2 "$APP"
hdiutil verify "$DMG"

rm -rf release
mkdir release
cp "$DMG" release/VYRON-2.0.1-macOS-AppleSilicon.dmg
cp "$UPDATER" release/VYRON.app.tar.gz
cp "$SIG" release/VYRON.app.tar.gz.sig
shasum -a 256 release/VYRON-2.0.1-macOS-AppleSilicon.dmg > release/SHA256.txt
shasum -a 256 release/VYRON.app.tar.gz > release/UPDATER_SHA256.txt
tar -czf release/VYRON-2.0.1-source.tar.gz --exclude='./node_modules' --exclude='./dist' --exclude='./src-tauri/target' --exclude='./release' .
shasum -a 256 release/VYRON-2.0.1-source.tar.gz > release/SOURCE_SHA256.txt

echo 'VYRON 2.0.1 signed build stage: PASS'
