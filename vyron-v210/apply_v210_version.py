...
if app.exists():
    s=app.read_text()
    s=s.replace('Версия 0.9.9.','Версия 2.0.10.')
    s=s.replace("update.current||'0.8.1'","update.current||'2.0.10'")
    s=s.replace('Версия <b>0.9.9</b>','Версия <b>2.0.10</b>')
    app.write_text(s)
print('VYRON YT PEISOV version -> 2.0.10')
