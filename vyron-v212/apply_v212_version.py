...
import json,re,sys
ROOT=Path(sys.argv[1] if len(sys.argv)>1 else '.');version='2.0.12'
for rel in ('package.json','src-tauri/tauri.conf.json'):
 p=ROOT/rel;x=json.loads(p.read_text());x['version']=version
...
if (ROOT/'package-lock.json').exists():
 ...
 p.write_text(json.dumps(x,ensure_ascii=False,indent=2)+'\n')
print('VYRON YT PEISOV version -> 2.0.12')
